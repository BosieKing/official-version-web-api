...
namespace IDataSphere.Extensions
{
    ...
    public static class LinqExtensions
    {
        ...
        public static IQueryable<DropdownResult> ConvertListSearch<TSource>(this IQueryable<TSource> sources, string fieldName = "")
        {
            ...
            // 赋值表达式，构建 Id => p.id
            MemberAssignment idMemberAssignment = Expression.Bind(resultType.GetProperty(nameof(DropdownResult.Id)), idMemberExpression);
            MemberAssignment nameMemberAssignment = Expression.Bind(resultType.GetProperty(nameof(DropdownResult.Value)), nameMemberExpression);
            // 规范返回模板
            ...
            // 转换为查询表达式
            var searchQuery = Expression.Lambda<Func<TSource, DropdownResult>>(memberInitExpression, p);
