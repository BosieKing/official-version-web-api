...
namespace DataSphere
{
    ...
    /// 自定义操作数据库方法
    /// </summary>
    public class Repository<T> : IRepository<T> where T : EntityBaseDO
    {
        #region 构造函数   
        ...
        {
            DbSet<TEntity> rep = _db.Set<TEntity>();
            return rep;
        }
        #endregion
        ...
        /// <typeparam name="TInput">输入参数类型，必须继承PageInput</typeparam>
        /// <typeparam name="TOutput">输出参数类型，必须继承PageOutput</typeparam>
        ...
        public async Task<bool> UpdateAsync<TEntity>(TEntity data) where TEntity : EntityBaseDO
        {
            ...
            try
            {
                DbSet<TEntity> rep = _db.Set<TEntity>();
                rep.Update(data);
                await _db.SaveChangesAsync(); transaction.Commit();
            }
            catch (Exception)
            {
                transaction.Rollback();
