...
namespace DataSphere.ES
{
    ...
    public class ElasticSearchHelper : IElasticSearchHelper
    {
        ...
        /// <returns></returns>
        public void PostIndexSetting(ConnectionSettings settings)
        {
            // 由于PostIndex存在父子文档
            ...
            // RoutingField(p => p.Required()) 指定路由字段值取PostIndexid，所有postindexid中具有相同值的都会被分配在同一个分片上
            ...
            settings.DefaultMappingFor<CommentChildIndex>(m => m.IndexName(parentIndeName));
