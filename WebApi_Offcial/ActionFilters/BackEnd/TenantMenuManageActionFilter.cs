...
namespace WebApi_Offcial.ActionFilters.BackEnd
{
    ...
    public class TenantMenuManageActionFilter : IAsyncActionFilter
    {
        ...
        {
            bool nameExisted = await _tenantMenuManageDao.SingleDataExisted<T_TenantDirectory>(p => p.Name == input.Name && p.BrowserPath == input.BrowserPath);
            if (nameExisted)
            {
        ...
        private async Task<ServiceResult> AddTenantMenuVerify(AddTenantMenuInput input)
        {
            ...
            bool nameExisted = await _tenantMenuManageDao.SingleDataExisted<T_TenantMenu>(p => p.Name == input.Name && p.BrowserPath == input.BrowserPath && p.ControllerRouter == input.ControllerRouter);
            if (nameExisted)
            {
                return ServiceResult.IsFailure(_stringLocalizer["DataExisted"].Value);
            }
        ...
        private async Task<ServiceResult> AddTenantMenuButtonVerify(AddTenantMenuButtonInput input)
        {
            ...
            bool nameExisted = await _tenantMenuManageDao.SingleDataExisted<T_TenantMenuButton>(p => p.Name == input.Name && p.ActionName == input.ActionName);
            if (nameExisted)
            {
                return ServiceResult.IsFailure(_stringLocalizer["DataExisted"].Value);
            }
        ...
        {
            bool nameExisted = await _tenantMenuManageDao.SingleDataExisted<T_TenantDirectory>(p => p.Name == input.Name && p.BrowserPath == input.BrowserPath && p.Id != input.Id);
            if (nameExisted)
            {
                return ServiceResult.IsFailure(_stringLocalizer["DataExisted"].Value);
            }
        ...
        private async Task<ServiceResult> UpdateTenantMeunVerify(UpdateTenantMenuInput input)
        {
            ...
            bool nameExisted = await _tenantMenuManageDao.SingleDataExisted<T_TenantMenu>(p => p.Name == input.Name && p.BrowserPath == input.BrowserPath && p.ControllerRouter == input.ControllerRouter && p.Id != input.Id);
            if (nameExisted)
            {
                return ServiceResult.IsFailure(_stringLocalizer["DataExisted"].Value);
            }
        ...
        private async Task<ServiceResult> UpdateTenantMenuButtonVerify(UpdateTenantMenuButtonInput input)
        {
            ...
            bool nameExisted = await _tenantMenuManageDao.SingleDataExisted<T_TenantMenuButton>(p => p.Name == input.Name && p.ActionName == input.ActionName && p.Id != input.Id);
            if (nameExisted)
            {
                return ServiceResult.IsFailure(_stringLocalizer["DataExisted"].Value);
            }
        ...
        {
            bool unbound = await _tenantMenuManageDao.SingleDataExisted<T_TenantMenu>(p => p.DirectoryId == input.Id);
            if (unbound)
            {
        ...
        {
            bool unboundByRole = await _tenantMenuManageDao.SingleDataExisted<T_RoleMenu>(p => p.MenuId == input.Id);
            if (unboundByRole)
            {
